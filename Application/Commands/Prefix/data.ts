...
module.exports = {
  ...
  execute: async (message: Message, client: Client) => {
    ...
    if (guild.id === "1348623820331679744") {
      if (AuthorizedRoles.some((roleId) => member.roles.cache.has(roleId))) {
        ...
        if (result.type === "user") {
          ...
          message.reply(`${emoji.x} This is not a valid user/guild id, please try again with a valid id.`);
        }
